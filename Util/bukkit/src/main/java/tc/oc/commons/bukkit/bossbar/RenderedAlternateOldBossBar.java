...
public class RenderedAlternateOldBossBar extends LegacyBossBar implements BossBar {

    private final ComponentRenderContext renderer;
    private final BossBarFactory bossBarFactory;

    private final Map<Player, NMSHacks.FakeEnderDragon> views = new HashMap<>();
    ...
    @Inject RenderedAlternateOldBossBar(ComponentRenderContext renderer, BossBarFactory bossBarFactory) {
    ...
    public void addPlayer(Player player) {
        if(!views.containsKey(player)) {
            NMSHacks.FakeEnderDragon view = new NMSHacks.FakeEnderDragon(player.getWorld(), true, renderer.renderLegacy(title, player));
    ...
    public void removePlayer(Player player) {
        NMSHacks.FakeEnderDragon view = views.remove(player);
        try {
    ...
    public Location getFakeEntityLocation(Player p) {
        ...
        if (pitch >= 55)
            ...
            l = l.getBlock().getRelative(getDirection(l), Bukkit.getServer().getViewDistance() * 16).getLocation();
